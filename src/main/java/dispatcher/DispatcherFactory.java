...
public class DispatcherFactory {
    ...
    private final int fanout, delay;
    private final TimeUnit delayTimeUnit;

    public DispatcherFactory(int port, DispatcherEventsRegister register, boolean isGossipDispatcher, int fanout, int delay, TimeUnit delayTimeUnit){
        this.port = port;
    ...
    public MessageDispatcher getMessageDispatcher(SimulatedPlatform platform){
        ...
        if(isGossipDispatcher)
            md = new MutatedGossipDispatcher(Address.from("127.0.0.1", port), register, platform, fanout, delay, delayTimeUnit);
        else
